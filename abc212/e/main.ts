...
function main() {
    ...
    const unusables: number[][] = [...Array(N)].map(() => []);
    for (let i = 0; i < N; i++) {
        unusables[i].push(i);
    }
    for (let i = 0; i < M; i++) {
        const u = input.number() - 1;
        const v = input.number() - 1;
        unusables[u].push(v);
        unusables[v].push(u);
    }

    // K 日目に都市 i にいる場合の組み合わせの数を dp[i] とする
    let dp = array(N, 0);
    // K - 1 日目に都市 i にいる場合の組み合わせの数を dp2[i] とする
    let dp2 = array(N, 0);
    dp[0] = 1;
    let sum = 1;
    for (let i = 0; i < K; i++) {
